namespace Lithnet.ResourceManagement.Client
{
    ...
    public class ResourceManagementClient
    {
        ...
        private static void InitialResourceManagementClientDefaults()
        {
            ...
            {
                spn =  new SpnEndpointIdentity(Configuration.ServicePrincipalName);
            }
        ...
        public ResourceManagementClient(Uri baseAddress, NetworkCredential credentials, string servicePrincipalName, bool allowNtlm)
        {
            ...
            {
                id = new SpnEndpointIdentity(servicePrincipalName);
            }
            else
            {
                id = new SpnEndpointIdentity(string.Format("FIMService/{0}", baseAddress.Host));
            }

            EndpointManager e = new EndpointManager(baseAddress, id);
            this.InitializeClients(e, credentials, allowNtlm).ConfigureAwait(false).GetAwaiter().GetResult();
        }
        ...
        /// </summary>
        private async Task InitializeClients(EndpointManager endpointManager, NetworkCredential credentials, bool allowNtlm)
        {
...
#pragma warning restore 0618
            ...
            await this.resourceClient.OpenAsync();
            await this.resourceFactoryClient.OpenAsync();
            await this.searchClient.OpenAsync();
